...
class OpportunityWindowEngine:
    def __init__(self, config: Dict):
        self.config = config

    def _smoothed(self, values: List[float], window: int) -> List[float]:
        if window <= 1:
            return list(values)
        out: List[float] = []
    ...
    def compute_time_series(self, features_by_day: List[Dict]) -> List[DailyScore]:
        norm = self.config.get("normalization", {}).get("opportunity", {"min": -4.0, "max": 8.0})
        smooth_days = int(self.config.get("window", {}).get("smoothing_days", 1))
        raw_values = [float(item.get("opportunity_raw", 0.0)) for item in features_by_day]
        smoothed = self._smoothed(raw_values, smooth_days)

        series: List[DailyScore] = []
        for idx, item in enumerate(features_by_day):
            val = smoothed[idx]
            series.append(
                DailyScore(
                    date=item["date"],
                    opportunity_raw=val,
                    risk_raw=float(item.get("risk_raw", 0.0)),
                    opportunity_score=_normalize(val, norm.get("min", -4.0), norm.get("max", 8.0)),
                    risk_score=0,
    ...
    def rank_candidates(self, time_series_scores: List[DailyScore]) -> List[WindowResult]:
        ...
        for length in range(min_days, max_days + 1):
            for start_idx in range(0, len(time_series_scores) - length + 1):
                ...
                avg = sum(item.opportunity_score for item in chunk) / float(length)
                positive_counter: Counter = Counter()
                evidence_map: Dict[str, str] = {}
                for item in chunk:
                    positive_counter.update(item.positive_drivers or item.drivers)
                    for label, snippet in item.evidence.items():
                        if label not in evidence_map and snippet:
                            evidence_map[label] = snippet

                top_drivers = [d for d, _ in sorted(positive_counter.items(), key=lambda x: (-x[1], x[0]))[:5]]
                details = []
    ...
    def detect(self, time_series_scores: List[DailyScore]) -> WindowResult:
        ...
        best = ranked[0]
        min_score = int(self.config.get("window", {}).get("min_opportunity_score", 45))
