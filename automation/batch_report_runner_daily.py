...
def run_batch(csv_path: Path, output_dir: Path, send_email_ind: bool = False) -> List[BatchRowResult]:
    ...
    for idx, row in enumerate(rows, start=1):
        try:
            ...
            print(f"Input: {req}")
            output = dailyWeeklyTimeline(req, day_or_week="daily", lang_code=lang_code)
            ...
            if send_email_ind and to_email and output:
                try:
                    try:
                        html_body = render_basic_forecast_html_daily(output.shortSummary)
                    except Exception:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Write automation/natal_inputs_for_daily_forecast.csv and run it to ./output",
    )
    parser.add_argument(
        "--send-email",
        action="store_true",
    ...
    if args.demo:
        sample_csv = Path(__file__).with_name("natal_inputs_for_daily_forecast.csv")
